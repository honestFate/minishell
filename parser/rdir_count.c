...
#include "minishell.h"
//замени в if > < чтобы сделать код аккуратнее
...
void	correct_count_utils(t_params *data, int *i)
{
	...
	{
		if (is_rdir(data->line[*i + 1]) && (is_nothing(data->line[*i + 2]) || data->line[*i + 2] == '\r'))
		{
			...
			data->node->n_rdir_arg += 2;
		else if (!is_nothing(data->line[*i + 1]) && !is_rdir(data->line[*i + 1]))
